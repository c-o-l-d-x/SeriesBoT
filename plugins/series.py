    ...
    if season_id and lang_id:
        ...
        buttons.extend([
            ...
            [
                ...
                InlineKeyboardButton("🗑️ Clear Episodes", callback_data=f"clearepisodes_{series_id}_{lang_id}_{season_id}")
            ],
            [InlineKeyboardButton("🗑️ Delete Season", callback_data=f"confirm_deleteseason_{series_id}_{lang_id}_{season_id}")],
        ...
        buttons.extend([
            ...
            [InlineKeyboardButton("🗑️ Delete Language", callback_data=f"confirm_deletelang_{series_id}_{lang_id}")],
        ...
        buttons.extend([
            ...
            [
                InlineKeyboardButton("🗑️ Delete Series", callback_data=f"confirm_delete_series_{series_id}"),
...
async def show_episode_admin_view(message_or_query, series_id, lang_id, season_id, episode_id):
    ...
    buttons.extend([
        ...
        [InlineKeyboardButton("\U0001f5d1\ufe0f Delete Episode", callback_data=f"confirm_deleteepisode_{series_id}_{lang_id}_{season_id}_{episode_id}")],
...
async def callback_handler(client: Client, callback_query: CallbackQuery):
    ...
    try:
        ...
        # Confirm delete season
            ...
            buttons = [[
                InlineKeyboardButton("✅ Confirm", callback_data=f"deleteseason_{series_id}_{lang_id}_{season_id}"),
                InlineKeyboardButton("❌ Cancel", callback_data=f"season_{series_id}_{lang_id}_{season_id}")
            ]]
            await callback_query.answer()
            try:
                await callback_query.message.edit_text(
                    f"⚠️ Are you sure you want to delete <b>'{season_name}'</b>?\nThis will delete all its episodes and qualities!",
                    reply_markup=InlineKeyboardMarkup(buttons)
                )
            except:
                await callback_query.message.reply_text(
        ...
        # Confirm clear episodes
            ...
            series_id, lang_id, season_id = parts
            buttons = [[
                InlineKeyboardButton("✅ Confirm", callback_data=f"doclrearepisodes_{series_id}_{lang_id}_{season_id}"),
                InlineKeyboardButton("❌ Cancel", callback_data=f"season_{series_id}_{lang_id}_{season_id}")
            ]]
            await callback_query.answer()
            try:
                await callback_query.message.edit_text(
                    f"⚠️ Are you sure you want to clear <b>all episodes</b> from this season?",
                    reply_markup=InlineKeyboardMarkup(buttons)
                )
            except:
                await callback_query.message.reply_text(
        ...
        # Do clear episodes
        elif data.startswith("doclrearepisodes_"):
            parts = data.replace("doclrearepisodes_", "").split("_", 2)
            series_id, lang_id, season_id = parts
        ...
        # Confirm delete episode
            ...
            buttons = [[
                InlineKeyboardButton("✅ Confirm", callback_data=f"deleteepisode_{series_id}_{lang_id}_{season_id}_{episode_id}"),
...
async def episode_file_handler(client: Client, message: Message):
    ...
    try:
        ...
        # Update episode admin view only (no duplicate series management message)
        try:
            main_msg = await client.get_messages(message.chat.id, state.message_id)
            class FakeQuery:
                def __init__(self, msg):
