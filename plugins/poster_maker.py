...
def _compress_for_upload(img_bytes: bytes) -> bytes:
    """
    Compress only when upload fails — progressive quality + resolution reduction.
    Preserves maximum clarity by trying quality steps before resizing.
    """
    img = Image.open(io.BytesIO(img_bytes)).convert("RGB")
    w, h = img.size

    # Step 1: quality ladder (85 -> 30, step -5) at original resolution
    for quality in range(85, 25, -5):
        out = io.BytesIO()
        img.save(out, format="JPEG", quality=quality, optimize=True)
        data = out.getvalue()
        logger.debug(f"Compress attempt: quality={quality}, size={len(data)//1024}KB")
        return data  # return first step — caller will retry upload

    # Step 2: fallback — reduce resolution too
    out = io.BytesIO()
    img.resize((int(w * 0.7), int(h * 0.7)), Image.LANCZOS).save(out, format="JPEG", quality=60, optimize=True)
    return out.getvalue()
...
def kb_logos(sid: str, uid: int, logos: list) -> InlineKeyboardMarkup:
    """Logo picker grid with a [🚫 None] button as the first option."""
    rows = []
    # None button — remove logo
    rows.append([InlineKeyboardButton("🚫 None (No Logo)", callback_data=f"pm_nonelogo_{sid}_{uid}")])
    # Numbered logo buttons, 5 per row
    row = []
    for idx in range(len(logos)):
        row.append(InlineKeyboardButton(str(idx + 1), callback_data=f"pm_sellogo_{sid}_{uid}_{idx}"))
        if len(row) == 5:
            rows.append(row)
            row = []
    if row:
        rows.append(row)
    rows.append([InlineKeyboardButton("« Back", callback_data=f"pm_back_{sid}_{uid}")])
    return InlineKeyboardMarkup(rows)
...
def _caption(series: dict, session: dict) -> str:
    ...
    if total_logo:
        logo_display = "None" if logo_idx == -1 else f"{logo_idx + 1}/{total_logo}"
        text += f"  |  <b>Logo:</b> {logo_display}"
...
async def _edit_poster(client: Client, msg: Message, series: dict, session: dict, sid: str, uid: int):
    ...
    logo_url = None if logo_idx == -1 or not logos else logos[logo_idx]
...
async def poster_callback(client: Client, cq: CallbackQuery):
    ...
    # ── Select specific image by index ──────────────────────────────────────
        ...
        # Stay on image picker — user can keep browsing; press Back when done
    ...
    # ── Select None logo (no logo overlay) ─────────────────────────────────
    elif action == "nonelogo":
        session["logo_idx"] = -1
        set_session(uid, session)
        await cq.answer("🚫 No logo selected")
        # Stay on logo picker — press Back when done
    ...
    # ── Select specific logo by index ───────────────────────────────────────
        ...
        # Stay on logo picker — user can keep browsing; press Back when done
    ...
    # ── Set position ─────────────────────────────────────────────────────────
        ...
        # Stay on position picker — press Back when done
    ...
    # ── Set size ─────────────────────────────────────────────────────────────
        ...
        # Stay on size picker — press Back when done
    ...
    # ── Done — upload & save ─────────────────────────────────────────────────
        ...
        # Try upload; compress and retry once if it fails
        poster_url = await upload_to_imgbb(tmp)
        if not poster_url:
            logger.warning("ImgBB upload failed, compressing and retrying...")
            await msg.edit_caption(caption="⏳ Upload failed, compressing and retrying...")
            compressed = _compress_for_upload(img_bytes)
            tmp2 = f"/tmp/poster_{sid}_{uuid.uuid4().hex[:8]}_c.jpg"
            with open(tmp2, "wb") as f:
                f.write(compressed)
            poster_url = await upload_to_imgbb(tmp2)
            try:
                os.remove(tmp2)
            except Exception:
                pass
