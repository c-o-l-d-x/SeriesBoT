...
async def check_force_sub(client: Client, message: Message) -> tuple[bool, str]:
    ...
    if await force_sub_db.is_user_authorized(user_id):
        # For request mode: trust the DB (join request already approved)
        if mode == "request":
            logger.info(f"User {user_id} is already authorized (request mode)")
            return True, None
        # For normal mode: DO NOT trust the DB — always live-check below
        # (user may have left the channel since they were last saved)
    ...
    # For normal mode, always live-check actual membership
    else:
        try:
            # Check if user is currently a member of the channel
            member = await client.get_chat_member(chat_id=channel_id, user_id=user_id)
            
            if member.status in [ChatMemberStatus.BANNED]:
                # Remove from DB if they were saved before
            ...
            if member.status in [ChatMemberStatus.MEMBER, ChatMemberStatus.ADMINISTRATOR, ChatMemberStatus.OWNER]:
                # User is a member — save/update in DB
            ...
            # Any other status (e.g. LEFT) — not a member
            raise UserNotParticipant("Not a member")
            
        except UserNotParticipant:
            # User left or never joined — remove from DB so they must re-join
            await force_sub_db.delete_user(user_id)
