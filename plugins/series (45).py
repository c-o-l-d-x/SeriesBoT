...
async def episode_file_handler(client: Client, message: Message):
    ...
    try:
        ...
        # Update the existing episode admin view message (edit in place)
        try:
            main_msg = await client.get_messages(message.chat.id, state.message_id)
            class FakeQuery:
                def __init__(self, msg):
            ...
            # Fallback: send a fresh episode admin view message
            try:
                await show_episode_admin_view(message, state.series_id, state.lang_id, state.season_id, state.episode_id)
            except Exception as e2:
                logger.error(f"Error sending fallback episode view: {e2}")
