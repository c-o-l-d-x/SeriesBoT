...
def compose_poster(backdrop_url: str, logo_url, position: int = 8, size: str = "medium"):
    ...
    try:
        ...
        rgb = bg.convert("RGB")
        return _compress_to_limit(rgb, max_kb=150)
...
def _compress_to_limit(img: Image.Image, max_kb: int = 150) -> bytes:
    """
    Save at quality=90. Only compresses if size exceeds max_kb (150KB limit).
    """
    max_bytes = max_kb * 1024

    # First try: high quality, no compression
    out = io.BytesIO()
    ...
    # Quality ladder if above limit
    for quality in range(85, 25, -5):
        out = io.BytesIO()
        img.save(out, format="JPEG", quality=quality, optimize=True)
        data = out.getvalue()
        if len(data) <= max_bytes:
            logger.debug(f"Poster compressed to {len(data)//1024}KB at quality={quality}")
            return data

    # Resolution downscale as last resort
    w, h = img.size
    for scale in [0.9, 0.8, 0.7, 0.6, 0.5]:
        ...
        for quality in range(85, 25, -5):
            ...
            data = out.getvalue()
            if len(data) <= max_bytes:
                logger.debug(f"Poster compressed to {len(data)//1024}KB at scale={scale} quality={quality}")
                return data

    # Last resort
    out = io.BytesIO()
    img.resize((int(w * 0.5), int(h * 0.5)), Image.LANCZOS).save(out, format="JPEG", quality=30, optimize=True)
    logger.warning(f"Poster could not reach {max_kb}KB, returning best effort")
    return out.getvalue()
...
def kb_logos(sid: str, uid: int, logos: list) -> InlineKeyboardMarkup:
    ...
    # Insert "No Logo" button at the very top
    no_logo_row = [InlineKeyboardButton("🚫 No Logo", callback_data=f"pm_sellogo_{sid}_{uid}_nologo")]
    return InlineKeyboardMarkup([no_logo_row] + grid.inline_keyboard)
...
async def _edit_poster(client: Client, msg: Message, series: dict, session: dict, sid: str, uid: int):
    ...
    logo_idx = session.get("logo_idx", 0)
    logo_url = session["logos"][logo_idx] if (session.get("logos") and logo_idx is not None) else None
...
async def poster_command(client: Client, message: Message):
    ...
    session = {
        ...
        "logos": logos,
        "bg_idx": 0,
        "logo_idx": None,  # Start with no logo — user can add one if needed
        "position": 8,
        "size": "medium",
    }
    ...
    # Compose initial poster with no logo overlay
    img_bytes = await asyncio.get_event_loop().run_in_executor(
        None, compose_poster,
        backdrops[0], None, 8, "medium"
    )
...
@Client.on_callback_query(filters.regex(r"^pm_"), group=-1)
