...
def _compress_to_limit(img: Image.Image, max_kb: int = 120) -> bytes:
    """
    Smart compression: starts at quality=90, steps down until size < max_kb.
    If still too large after quality hits minimum (30), downsizes resolution
    progressively (keeping 16:9) until it fits — preserving as much clarity
    as possible.
    """
    max_bytes = max_kb * 1024

    # Step 1: quality ladder (90 -> 30, step -5)
    ...
    # Step 2: resolution downscale (keep 16:9, step down by 10%)
    w, h = img.size
    ...
    # Step 3: last resort - return smallest we got
    out = io.BytesIO()
