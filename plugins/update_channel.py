...
async def send_or_update_series_message(client: Client, series_id: str):
    """
    Edit existing update message or create new one when publish button is clicked.
    ...
    try:
        ...
        if update_msg_id:
            ...
            try:
                ...
                logger.info(f"Updated message for '{series.get('title')}'")
                return True
            except Exception as e:
                logger.error(f"Failed to edit message {update_msg_id}: {e}")
                # If edit fails, message might be deleted - create new one
                update_msg_id = None
        
        # Create new message only if no message exists
        if not update_msg_id:
            try:
                ...
                logger.info(f"Created new message for '{series.get('title')}'")
                return True
